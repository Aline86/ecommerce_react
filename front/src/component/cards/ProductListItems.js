...
const ProductListItems =({product}) => {
    ...
    return (
        <ul className="list-group">
                ...
                </li>)}
            {
                subs && (
                    <li className="list-group-item">
                        Sub Categories
                        {subs.map((s) => (
                            <Link key={s._id} to={`/sub/${s.slug}`} className="float-right ml-3">{s.name}</Link>
                        ))}
                    </li>
                )
            }
            ...
            <li className="list-group-item">
                Available{" "} <span className="float-right">{quantity}</span>
            </li>
            <li className="list-group-item">
                Sold{" "} <span className="float-right">{price}</span>
            </li>
        </ul>
    )
}
